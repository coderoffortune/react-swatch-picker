...
const SwatchPicker = ({color, colorList, closeOnPick, onColorSelect}) => {
    ...
    const pickColor = color => {
        ...
        closeOnPick && setShowPicker(false)
    }

    return (
        <div className="swatch-picker-container" ref={wrapperRef}>
            <IconButton onClick={togglePicker}>
                <SwatchPickerIcon htmlColor={selectedColor} width={240} height={240} />
            </IconButton>
            <Paper className={!showPicker ? 'swatch-picker closed': 'swatch-picker' } elevation={4}>
            {
                (colorList || defaultColors).map(color => 
                    <div className="swatch-color" key={color} style={{backgroundColor: color}} onClick={() => {pickColor(color)}} ></div>
                )
            }
            </Paper>
        </div>
